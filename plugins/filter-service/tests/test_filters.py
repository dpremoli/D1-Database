...
def synth(n=200_000, seed=3):
    ...
    t = np.arange(n) / FS
    f0 = RPM / 60
    clean = 50 + 0.5 * np.sin(2 * np.pi * 0.7 * t)  # slow
...
def test_despike_removes_spikes():
    ...
    out, sk = apply_chain({"Fz": x}, FS, RPM, chain)
    assert not sk
    assert np.max(np.abs(out["Fz"][spikes] - x[spikes])) > 20
    assert np.abs(out["Fz"] - x).mean() < 0.1


def test_notch_kills_spindle_tone():
    t, x, _, _ = synth()
    f0 = RPM / 60
    chain = {"notch": {"on": True, "harmonics": [1], "q": 30}}
    out, sk = apply_chain({"Fz": x}, FS, RPM, chain)
    assert not sk

    # project onto the tone: amplitude at f0 should collapse
    def proj(y):
        return 2 * np.abs(np.mean((y - y.mean()) * np.exp(-2j * np.pi * f0 * t)))
...
def test_highpass_removes_drift_keeps_ac():
    ...
    out, _ = apply_chain({"Fz": x}, FS, RPM, chain)
    y = out["Fz"]
...
def test_preview_nyquist_skip():
    ...
    chain = {"lowpass": {"on": True, "cutoff_hz": 2000, "order": 4}}
    out, sk = apply_chain({"Fz": x}, 1000.0, RPM, chain)
    assert len(sk) == 1 and "Nyquist" in sk[0]
    assert np.array_equal(out["Fz"], x)


def test_validation_errors():
    x = np.zeros(100)
    with pytest.raises(ChainError):
        chain = {"despike": {"on": True, "window": 10, "sigma": 5}}
        apply_chain({"Fz": x}, FS, RPM, chain)
    with pytest.raises(ChainError):
        chain = {"notch": {"on": True, "harmonics": [], "q": 30}}
        apply_chain({"Fz": x}, FS, RPM, chain)
...
def test_d1lc_roundtrip_and_stride():
    n = 1000
    c = Cache(
        FS, 0.1, 80.0, 1.0, 2.0, *(np.arange(n, dtype=np.float32) + k for k in range(6))
    )
    buf = serialise(c, 1)
    c2 = parse(buf)
