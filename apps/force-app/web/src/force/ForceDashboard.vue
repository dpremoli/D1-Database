...
// Standalone: the record editors live in Directus admin, not in this app — open them there
// in a new tab (the embedded module used an in-app router.push to the Directus content route).
function openSampleForm() { if (sampleInfo.value?.id) window.open(`${getConfig().directusUrl}/admin/content/physical_samples/${sampleInfo.value.id}`, '_blank', 'noopener'); }
function openOpForm() { if (op.value?.operation_id) window.open(`${getConfig().directusUrl}/admin/content/manufacturing_operations/${op.value.operation_id}`, '_blank', 'noopener'); }
...
// Standalone download: assets are cross-origin and Bearer-authed, so a plain <a href> can't
// carry the token. Fetch the asset through the authenticated client and save the blob.
