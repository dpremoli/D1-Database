...
async function load() {
	...
	// Cross-origin octree server (CORS-enabled, network-trusted). Base is configured, not the
	// SPA origin, since the standalone app is hosted separately from the octree host.
	const base = `${getConfig().octreeUrl}/${props.octreePath}/`;
	try {
		await loadMeta(base);
		potree = new Potree();
