...
people_fields = [
    ...
    (
        ...
        "m2o",
        "select-dropdown-m2o",
        '{"template":"{{first_name}} {{last_name}}","enableCreate":false}',
        "related-values",
        "half",
        3,
...
for (
    ...
    w(
        f"INSERT INTO directus_fields (collection, field, special, interface, options, display, readonly, hidden, sort, width, required, note) VALUES "
...
for field in ("legacy_machine_operator_id",):
    w(
        f"INSERT INTO directus_fields (collection, field, hidden, readonly, sort, note) VALUES ('people', '{field}', TRUE, TRUE, 90, 'Legacy Machine_Operators.id (provenance).');"
    )
...
def person_m2o(collection, field, label, sort):
    ...
    trans = f'[{{"language": "en-US", "translation": "{label}"}}]'
    w(
        f"INSERT INTO directus_fields (collection, field, special, interface, options, display, display_options, readonly, hidden, sort, width, required, translations) VALUES "
        f"('{collection}', '{field}', 'm2o', 'select-dropdown-m2o', '{opts}', 'related-values', '{disp}', FALSE, FALSE, {sort}, 'half', FALSE, '{trans}');"
    )
    w(
        f"INSERT INTO directus_relations (many_collection, many_field, one_collection, one_deselect_action) "
...
w(
    f"UPDATE directus_fields SET hidden = TRUE WHERE (collection, field) IN ({owner_pairs});"
)
w(
...
w(
    f"UPDATE directus_fields SET hidden = FALSE WHERE (collection, field) IN ({owner_pairs});"
)
w(
...
with open(
    "db/migrations/20260703000062_people_directus_meta.sql", "w", encoding="utf-8"
) as f:
    f.write("\n".join(L) + "\n")
