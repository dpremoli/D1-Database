...
HEADERS = {
    "facing pass id": "pass_id",
    "facing pass #": "pass_num",
    "workpiece id": "workpiece",
    "workpiece information": "workpiece_info",
    "program (g96 / g97)": "program",
    "facing force data file id": "force_file_id",
    "vc [m/min]": "vc",
    "max rpm": "rpm",
    "fz [mm/rev]": "fz",
    "axial [mm]": "axial",
    ...
    "tacho (y/n)": "tacho",
    "coolant (y/n)": "coolant",
    "coolant pressure [bar]": "coolant_bar",
    "chips collected (y/n)": "chips",
    "sem captured": "sem",
    "cut time [min]": "cut_time",
    "captured with": "captured_with",
    "freq. [khz]": "freq",
    "notes": "notes",
}
...
def clean_str(v):
    ...
    return (
        s if s and s.lower() not in ("none", "n/a", "na", "n/s", "#n/a", "`") else None
    )
        ...
        for c in r:
            if c is not None and hnorm(c) == "facing pass id":
                return i, {
                    ...
                    if c is not None and hnorm(c) in HEADERS
                }
        ...
        out.append(
            {
                ...
                "vc": clean_float(cell(r, "vc")),
                "rpm": clean_float(cell(r, "rpm")),
                "fz": clean_float(cell(r, "fz")),
                "axial": clean_float(cell(r, "axial")),
                "diameter": clean_float(cell(r, "diameter")),
                "tool": clean_str(cell(r, "tool")),
                "insert": clean_str(cell(r, "insert")),
                "edge": clean_str(cell(r, "edge")),
                "new_edge": yn(cell(r, "new_edge")),
                "tacho": yn(cell(r, "tacho")),
                "coolant": yn(cell(r, "coolant")),
                "coolant_bar": clean_float(cell(r, "coolant_bar")),
                "chips": yn(cell(r, "chips")),
                "sem": clean_str(cell(r, "sem")),
                "cut_time": fmt_cut_time(cell(r, "cut_time")),
                "captured_with": clean_str(cell(r, "captured_with")),
                "freq": clean_float(cell(r, "freq")),
...
def build(row, campaign_id):
    ...
    insert_raw = " / ".join(
        filter(None, [row["insert"], row["edge"] and f"edge:{row['edge']}"])
    )
    notes = " | ".join(
        filter(
            None,
    ...
    try:
        ...
        pass_num = None
    return {
        "source_run_uid": hashlib.sha1(f"{campaign_id}|{pid}".encode()).hexdigest()[
            :16
        ],
        "operation_id": str(uuid.uuid5(_NS, f"{campaign_id}|{pid}")),
        "subtype": "MT-R" if is_rough else "MT-F",
        "pass_code": op_code,
        "force_file_id": op_code,
        "operation_sequence": pass_num,
        "operation_date": date_of(row["workpiece"]),
        "workpiece": row["workpiece"],
        "alloy": alloy_of(row["workpiece"]),
        "tool": row["tool"],
        "insert_raw": insert_raw or None,
        "edge": row["edge"] or row["insert"],
        "legacy_uid": pid,
        "vc": row["vc"],
        "rpm": row["rpm"],
        "fz": row["fz"],
        ...
        "tacho": row["tacho"],
        "coolant": row["coolant"],
        "coolant_bar": row["coolant_bar"],
        "chips": row["chips"],
        "force_captured": op_code is not None and not is_rough,
        "captured_with": row["captured_with"],
        "freq": row["freq"],
        "outcome_notes": notes,
    }
...
def main():
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT project_id, owner FROM campaigns WHERE campaign_id = %s",
        (args.campaign_id,),
    )
    ...
    ops = list(
        {
            o["source_run_uid"]: o
            for o in (build(r, args.campaign_id) for r in read_rows(args.path))
        }.values()
    )
    facing = sum(1 for o in ops if o["subtype"] == "MT-F")
    print(
        f"Parsed {len(ops)} unique passes — facing {facing}, roughing {len(ops) - facing}"
    )
    ...
    print(
        f"  tools matched:   {tools_m}/{sum(1 for o in ops if o['tool'])}  (unmatched: {sorted(un_tools)})"
    )
    print(
        f"  samples matched: {samples_m}/{len(ops)}  (unmatched: {sorted(un_samples)})"
    )
    ...
    cols = [
        ...
        "tool_id",
        "insert_edge_id",
        "machining_operation_subtype",
        "pass_code",
        "force_file_id",
        "operation_sequence",
        "machining_cutting_speed_m_per_min",
        "machining_spindle_speed_rpm",
        "machining_feed_mm_per_rev",
        "machining_axial_depth_of_cut_mm",
        "machining_workpiece_diameter_mm",
        "machining_new_edge",
        "machining_tacho_used",
        "machining_coolant_used",
        "machining_coolant_pressure_bar",
        "machining_chips_collected",
        "machining_force_captured",
        "capture_software",
        "capture_frequency_khz",
        "machining_legacy_insert_edge_id",
        "machining_legacy_machining_uid",
        "outcome_notes",
    ]
