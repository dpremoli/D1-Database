...
qa_logs['datetime'] = pd.to_datetime(qa_logs['Date'].astype(str) + ' ' + qa_logs['Time'].astype(str))
```
...
while offset < len(his_data) - 4:
    value = struct.unpack('<f', his_data[offset:offset+4])[0]  # Little-endian float32
    floats.append(value)
    offset += 4
    ...
    for i in range(min(10, len(runs['VersuchNr']))):  # First 10 runs as sample
        ...
        try:
            with ZipFile(emd_path) as zf:
                # Read HIS timeseries
                his_data = np.frombuffer(zf.read(f"{emd_file}.HIS")[4:], dtype=np.float32)
