...
def read_fast250_csv(csv_path):
    ...
    # Extract recipe name
    recipe = line2.split(':')[1].strip().replace('.rcp', '')  # "D105_IN718_Briq_1125_35MPa_30m"
...
def normalize_fast250_run(csv_path, run_id):
    ...
    df['datetime'] = pd.to_datetime(
        result['start_time'].date().astype(str) + ' ' + 
        df['Cur. Time Charge']
    )
    
    # Convert process time to float (handles "0,0003" → 0.0003)
    df['process_time_s'] = pd.to_numeric(
        df['Prozesstime 1'],
        errors='coerce'
    )
...
for seg in segments:
    print(f"  Segment {seg['segment_no']}: {seg['duration_s']}s @ {seg['temp_setpoint_c']}°C")
```
...
def parse_recipe_filename(rcp_path):
    ...
    match = re.search(r'D\d+_([A-Za-z0-9\-]+)_([A-Za-z]+)_(\d+)_(\d+)MPa_(\d+)m', filename)
    if match:
    ...
    match = re.search(r'AT_D\d+_([A-Za-z0-9\-]+)_(\d+)°C_(\d+)MPa_(\d+)m_(\d+)°\w+', filename)
    if match:
        result['material'] = match.group(1)
